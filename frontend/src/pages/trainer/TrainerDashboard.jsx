...
const TrainerDashboard = () => {
  ...
  const getColumnsConfig = (role) => {
    const techCol = {
      ...
      isInteractive: role === 'Technical Trainer' || role === 'Admin' || role === 'Super Admin'
    };
    const commCol = {
      key: 'communication',
      header: 'Communication Skills',
      isInteractive: role === 'Communication Trainer' || role === 'Admin' || role === 'Super Admin'
    };
    const aptiCol = {
      key: 'aptitude',
      header: 'Aptitude & Reasoning',
      isInteractive: role === 'Aptitude Trainer' || role === 'Admin' || role === 'Super Admin'
    };
  ...
  const getExportDataForStudent = (student) => {
  ...
  const exportAttendanceToExcel = () => {
    ...
    const dataToExport = filteredAttendanceStudents.map(student => getExportDataForStudent(student));
  ...
  const exportAttendanceToCSV = () => {
    ...
    const cleanBatchLabel = batchLabel.replace(/[^a-zA-Z0-9_-]/g, '_');
    downloadCSV(dataToExport, `Attendance_Report_${cleanBatchLabel}_${attendanceDate}.csv`, headers);
  };
